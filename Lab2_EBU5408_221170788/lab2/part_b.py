...
# window = np.hanning(frame_size) # Windowing is not needed for ICA preprocessing here
...
output_folder = 'Audio_output/PartB'
image_output = os.path.join('image_output', 'PartB')
...
for file_path in file_paths:
    ...
    # Example low-pass filter (if needed, e.g. for noise reduction)
    # butter_order = 5
    # lowpass_cutoff = 0.1  # Normalized frequency (0 to 1, 1 is Nyquist)
    # b_lp, a_lp = butter(butter_order, lowpass_cutoff, btype='low', output='ba')
